//
//  PL_ProvidenceIndentTextField.m
//  kkcoin
//
//  //
//
...
//
// PL_Providenceghostlord on 2018/4/12.
//  Copyright © KKCOIN. All rights reserved.
//

#import "PL_ProvidenceIndentTextField.h"
#import <objc/message.h>
@implementation PL_ProvidenceIndentTextField
...
- (void)dealloc {
    // 移除观察者
    [self PL_ProvidenceremoveNotification];
    NSLog(@"PL_ProvidenceIndentTextField dealloc --- %p",self);
}
...
- (void)setPlaceholderColor:(UIColor *)placeholderColor {
    ...
    if(placeholderColor && [placeholderColor isKindOfClass:[UIColor class]]) {
        ...
        // 更新placeHolder
        [self PL_ProvidenceupdatePlaceHolderColorWithPlaceHolder:self.placeholder];
    }
}
...
- (void)setPlaceholder:(NSString *)placeholder {
    ...
    if (placeholder) {
        [self PL_ProvidenceupdatePlaceHolderColorWithPlaceHolder:placeholder];
    }
}
...
- (void)PL_Providenceinitialize {
    ...
    // 添加观察者
    [self PL_ProvidenceaddNotification];
}

/**
 添加观察者
 */
- (void)PL_ProvidenceaddNotification {
    // 防止重复注册
    [self PL_ProvidenceremoveNotification];
    
    // 文字已经改变
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(PL_ProvidencetextDidChange:) name:UITextFieldTextDidChangeNotification object:nil];
    
    // 开始编辑
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(PL_ProvidencetextDidBeginEditing:) name:UITextFieldTextDidBeginEditingNotification object:nil];
    
    // 结束编辑
    [[NSNotificationCenter defaultCenter] addObserver:self selector:@selector(PL_ProvidencetextDidEndEditing:) name:UITextFieldTextDidEndEditingNotification object:nil];
}
...
/**
 移除观察者
 */
- (void)PL_ProvidenceremoveNotification {
...
/**
 设置placeHolder
 */
- (void)PL_ProvidenceupdatePlaceHolderColorWithPlaceHolder:(NSString *)placeHolder {
...
/**
 文字已经改变
 */
- (void)PL_ProvidencetextDidChange:(NSNotification *)notification {
    ...
    if(_PL_ProvidenceIndentTextFieldDelegate && [_PL_ProvidenceIndentTextFieldDelegate respondsToSelector:@selector(textField:textDidChangeWithUserInfo:)]) {
        [_PL_ProvidenceIndentTextFieldDelegate textField:self textDidChangeWithUserInfo:info];
    }
}
...
/**
 开始编辑
 */
- (void)PL_ProvidencetextDidBeginEditing:(NSNotification *)notification {
    if (![notification.object isEqual:self]) {
        return;
    }
    ...
    if(_PL_ProvidenceIndentTextFieldDelegate && [_PL_ProvidenceIndentTextFieldDelegate respondsToSelector:@selector(textField:textDidBeginEditingWithUserInfo:)]) {
        [_PL_ProvidenceIndentTextFieldDelegate textField:self textDidBeginEditingWithUserInfo:info];
    }
}
...
/**
 结束编辑
 */
- (void)PL_ProvidencetextDidEndEditing:(NSNotification *)notification {
    ...
    if(_PL_ProvidenceIndentTextFieldDelegate && [_PL_ProvidenceIndentTextFieldDelegate respondsToSelector:@selector(textField:textDidEndEditingWithUserInfo:)]) {
        [_PL_ProvidenceIndentTextFieldDelegate textField:self textDidEndEditingWithUserInfo:info];
    }
}
