...
static NSString *const PL_ProvidenceMarketListViewCell_id_1 = @"PL_ProvidenceMarketListViewCell_id_1";

@interface PL_ProvidenceHomeStockView ()<UITableViewDelegate,UITableViewDataSource>
...
@implementation PL_ProvidenceHomeStockView
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath {
    ...
    PL_ProvidenceHomeStockCell *cell = [tableView dequeueReusableCellWithIdentifier:PL_ProvidenceMarketListViewCell_id_1 forIndexPath:indexPath];
...
- (void)PL_ProvidenceloadDataFromNet {
    ...
    if ([self.identifier isEqualToString:@"fav"]) {
        ...
        [PL_ProvidenceMarketLogic getMarketListDateWithCode:self.identifier page:self.page success:^(NSArray<PL_ProvidenceHomeStockModel *> * _Nonnull list) {
...
- (UITableView *)listView {
    if (!_listView) {
        ...
        [_listView registerClass:[PL_ProvidenceHomeStockCell class] forCellReuseIdentifier:PL_ProvidenceMarketListViewCell_id_1];
...
- (UIButton *)refreshBtn {
    if (!_refreshBtn) {
        ...
        if ([self.identifier isEqualToString:@"fav"]) {
            [_refreshBtn setTitle:@"快去添加股票吧" forState:UIControlStateNormal];
        }else {
